...
def plot_roc_auc(
    clf_list: List[Tuple], X_test: np.ndarray, y_test: np.ndarray
) -> plt.figure:
    "PLots models roc auc curve"

    fig, ax = plt.subplots(figsize=(8, 6))

    for model, title in clf_list:

        y_predicted = model.predict_proba(X_test)[:, 1]
        fpr, tpr, _ = roc_curve(y_test, y_predicted)
        roc_score = roc_auc_score(y_test, y_predicted)
        ax.plot(
            fpr, tpr, lw=3, label=f"{title}, Roc Auc = {roc_score:.3f}",
        )
    ...
    ax.legend(
        fontsize=13, loc="lower right", ncol=1, facecolor="white", edgecolor="white",
    )
    ax.set_xlim([-0.01, 1.00])
    ax.set_ylim([-0.01, 1.01])

    ax.set_title(
        "Different classifiers Area under the curve", fontsize=14, y=1.02,
    )
...
def heat_map(
    ...
    ax = sns.heatmap(df, mask=mask, vmin=vmin, vmax=1, annot=True, cmap=cmap, fmt=fmt)
