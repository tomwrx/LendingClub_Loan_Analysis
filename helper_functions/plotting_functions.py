...
def plot_single_violin(
    df: pd.DataFrame, x: str, y: str, x_label: str, y_label: str, title: str
) -> plt.figure:
    "Plots a single violin plot"

    sns.catplot(x=x, y=y, data=df, cut=1.2, kind="violin", height=8, aspect=2,)
...
def cat_plot(
    df: pd.DataFrame, x: str, y: str, y_label: str, title: str, hue: str = None
) -> plt.figure:
...
def catplot_with_pct_2(
    ...
    for i, ax in enumerate(g.axes.flat):
        ax.set_title(f"{subplot_titles[i]}s", fontsize=12)
    ...
    g._legend.set_title(f"{hue.split('_')[0].capitalize()}?")
    g.fig.suptitle(
        title, fontsize=13, y=1.03,
    )
